    ...
    <q-page padding style="overflow-x: hidden;">
        ...
        <div v-else>
            <div class="row">
                <div class="col-3 q-mr-sm">
                    <q-img :src="imgPath" class="movie-poster">
                        <span :style="{ 'background-color': ratingColor }" class="text-black absolute-bottom-right q-ma-sm"
                            style="border-radius: 8px; padding: 0.5em 1em;">{{
                                movie.vote_average.toFixed(2)
                            }}</span>

                        <template v-slot:error>
                            <q-img class="movie-poster" fit="cover"
            ...
            <div class="flex items-center" style="flex-direction: column;">
                <h3 class="q-pa-sm" style="margin-right: auto; margin-bottom: 0; font-weight: 600;
    font-size: 1.4em;">{{ $t('mainActors') }}</h3>
                <actors-carousel />
            </div>
            <video-player />
    ...
    watch: {
        id: {
            handler: async function () {
                this.skeleton = true
                this.movie = await this.movieStore.FETCH_SINGLE_MOVIE_INFORMATION(this.id)
                this.skeleton = false
            },
            immediate: true
        }
