<template>
    <div v-for="item in listItems" :key="item" class=" item">
        <span v-if="item.field === 'home' || item.field === 'favorites'" @click="goToPage(item.field)"
            class=" flex items-center cursor-pointer q-pa-sm">{{
                item.name
            }}</span>
        <span v-else class="flex items-center cursor-pointer q-pa-sm" @click="setShowMenu">{{
            item.name }}</span>
    </div>
    <div class="menu-wrapper" v-if="showMenu" @mouseleave="setShowMenu">
        <div class="q-pa-md flex wrap item-2 cursor-pointer">
            <span v-for="genre in movieStore.genres" :key="genre.id" @click="goToPage('category', genre.id)"
                class="q-pa-sm">{{
                    genre.name
                }}</span>
        </div>
...
export default defineComponent({
    ...
    computed: {
        listItems() {
            return [{ name: this.$t('homePage'), field: 'home' }, { name: this.$t('favoritePage'), field: 'favorites' }, { name: this.$t('genres') }]
        }
    }
})
</script>
