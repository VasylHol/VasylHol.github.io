    ...
    <div class="col flex" style="
        ...
        <div>
            <div class="flex items-center">
                ...
                <q-space></q-space>
                <q-icon class="cursor-pointer" size="24px" color="white"
                    :name="isFavoriteMovie ? 'fa-solid fa-heart' : 'fa-regular fa-heart'" @click="addToFavorite(movie)">
        ...
        <span class="q-py-md " style="font-size: 1.3em; line-height: initial;">{{ movie.overview }}</span>
    </div>
</template>
    ...
    methods: {
        addToFavorite(movie) {
            if (!this.isFavoriteMovie) {
                ...
                this.movieStore.favoriteMovies = this.movieStore.favoriteMovies.filter(el => el.id !== movie.id)
            }
        }
    },
    computed: {
        isFavoriteMovie() {
            return !!this.movieStore.favoriteMovies.filter(el => el.id === this.movie.id)?.[0]
        },
        details() {
            return {
