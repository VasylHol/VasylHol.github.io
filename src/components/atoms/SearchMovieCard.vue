<template>
    <q-item class="movie-card">
        <div class="row wrap cursor-pointer" style="width: 100%;" @click="$emit('goToMovie', item)">
            <q-img :src="imgPath" class="poster" fit="cover">
                <template v-slot:error>
                    <q-img class="poster" fit="cover" :src="require('../../../public/icons/image-not-found.jpg')">
                    </q-img>
                </template>
            </q-img>
            <div class="column justify-between q-pa-sm" style="height: 150px;">
                <div class="title">
                    <span class="text-weight-bold" style="font-size:18px">{{ item.title }}
                    </span>
                    <br />
                    <span style="font-size:12px">{{ item.overview.slice(0, 120) + '...' }}</span>
                </div>
                <div class="flex justify-between items-end">
                    <span>{{ $t('votes') }}: {{ item.vote_count }}</span>
                    <span>
                        <q-icon name="star" size="sm" class="self-end q-mr-sm" color="yellow"
                            style="margin-left:auto"></q-icon>
                        <span :style="{ 'background-color': ratingColor }" class="text-black self-end"
