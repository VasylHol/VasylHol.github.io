<template>
    <div class="row items-center justify-center" ref="scrollComponent">
        <q-card v-for="movie in moviesList" :key="movie.id" class="item text-dark cursor-pointer"
            style="flex-direction: column;" @click="goToMovie(movie)">
            <div class="flex  items-center justify-center">
                <q-img :src="imagePath(movie.poster_path)" height="350px" width="250px">
                    ...
                    <div class="absolute-top-right" style="background: transparent;"><span class="text-dark"
                            :style="{ 'background-color': colorSchema[Math.round(movie.vote_average)], }"
            ...
            <span class="inline-block" style="font-size: 12px; padding: 2px; " v-for="movieGenre in movie[genresField]"
                :key="movieGenre">
...
export default defineComponent({
    ...
    methods: {
        handleScroll(e) {
            ...
            if (window.scrollY + window.innerHeight >= document.body.scrollHeight && this.movieStore.popularMovies.page !== nextPage && nextPage <= this.movieStore.popularMovies.total_pages) {
    ...
    async mounted() {
        this.requestParams = { ...(this.isCategoryPage && { 'with_genres': this.categoryId }) }
